...
from model_bone import SGN_Bone
...
def main():
    ...
    model = SGN_Bone(args.num_classes, args.dataset, args.seg, args)
    ...
    checkpoint = osp.join(save_path, '%s_ASN-T_1_best.pth' % args.case)
    earlystop_cnt = 0
    csv_file = osp.join(save_path, '%s_ASN-T_1_log.csv' % args.case)
    log_res = list()

    lable_path = osp.join(save_path, '%s_ASN-T_1_lable.txt' % args.case)
    pred_path = osp.join(save_path, '%s_ASN-T_1_pred.txt' % args.case)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (inputs, target) in enumerate(train_loader):
        ...
        # backward
        optimizer.zero_grad()  # clear gradients out bef ore each mini-batch
        loss.backward()
        optimizer.step()
...
def test(test_loader, model, checkpoint, lable_path, pred_path):
    ...
    # weights = OrderedDict(
    #     [[k.split('module.')[-1],
    #       v.cuda(output_device)] for k, v in weights.items() if k in model.state_dict()])
    model.load_state_dict(torch.load(checkpoint)['state_dict'])
    # try:
    #     model.load_state_dict(weights)
    #     print('load weights already!')
    # except:
